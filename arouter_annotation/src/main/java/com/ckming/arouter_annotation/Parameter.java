...
@Target(ElementType.FIELD)  //该注解作用在类之上 仅字段有作用
@Retention(RetentionPolicy.CLASS) //要在编译时进行一些预处理操作 注解会在class文件中存在
public @interface Parameter {

    String name() default "";   //从getIntent() 方法中获取传递参数值，不填写name默认该属性名为key

    /**
     * @Parameter(name = "Par")   Par则为该属性的key
     * String sex;
     *
     * @parameter       //通常写法
     * String sex;      不填写则默认以属性为key,即sex
     *
     */
}
